...
//  var canvas = createCanvas(windowWidth,windowHeight);
  ...
  scene0.resize(canW,canH-30);
  scene1.resize(canW,canH+270);
  scene2.resize(canW,canH+100);
  backgroundScene0 = createSprite(canW/2, 30+canH/2, canW, canH);
  ...
  backgroundScene1 = createSprite(canW+canW/2, 30+canH/2, canW, canH);
  ...
  gameOverImage.resize(canW,canH);
  gameOver.scale  = 1;
  gameOver.visible = false;
/**/ 
  boy = createSprite(60, canH-750, 20,60);
  boy.addAnimation("Joe",boyImage);
  boy.scale = 1.5;
 boyColliderLine = createSprite(50,canH-120,100,10);
 boyColliderLine.visible = false;
 //boyColliderLine = createSprite(80,height-110,200,10);
    imageMode(CENTER);
    restart = createSprite(canW/2+100,canH/2+5000);
    restart.addImage(restartImg);
    restart.scale = 1;  
    restart.visible = false;

    trophy = createSprite(canW/2, canH/2,50,50);
    trophy.addImage(trophyImg);
    trophy.scale=7;
    trophy.visible= false;

    wonBg = createSprite(canW/2, canH/2, canW, canH);
...
function draw()
{
   ...
   /* if(keyDown("space") && boy.y>=300)
    ...
    if(backgroundScene0.x<=-canW/2)
    {
         backgroundScene0.x = backgroundScene1.x+canW;
    }
    if(backgroundScene1.x<=-canW/2)
    {
         backgroundScene1.x = backgroundScene0.x+canW;
    }
...
//var r=3;
  ...
  if(score%5 == 0)  
  {
    if(l == "level0")
        ...
        {
    boyColliderLine.y = canH-60;
    backgroundScene0.y= canH-400;
    backgroundScene1.y = canH- 400;
...
imageMode(CENTER);
 ...
 gameWin.scale = 1.25;*/
  ...
  textFont("Georgia");
  fill("black");
  text("Congratulations",canW/2-100,80);
  text("You won!!!",canW/2-50,330);
...
function createCoin()
{
   ...
   {
        case 1:
            coin = createSprite(canW,canH-200, 10, 10);
            break;
        case 2: 
             coin = createSprite(canW,canH-250, 10, 10);
             break;

        case 3: 
             coin = createSprite(canW,canH-300, 10, 10);
             break;

        case 4: 
             coin = createSprite(canW,canH-350, 10, 10);
             break;
...
function createBird()
{
   ...
   // bird.lifetime =(width+100)/5;
    ...
    {
        case 1:
            bird = createSprite(canW,canH-350,10,10);
                break;

        case 2:  bird = createSprite(canW,canH-300,10,10);
                break;
        
        case 3:  bird = createSprite(canW,canH-250,10,10);
                break;

        case 4:  bird = createSprite(canW,canH-200,10,10);
                break;
        
        case 5:  bird = createSprite(canW,canH-400,10,10);
                break;
